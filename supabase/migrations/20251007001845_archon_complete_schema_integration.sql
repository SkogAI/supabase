-- =====================================================
-- Archon Complete Database Setup (Vendor Integration)
-- =====================================================
-- This migration integrates the complete Archon schema into our Supabase project.
-- Archon provides knowledge management, vector search, project/task management,
-- and MCP (Model Context Protocol) server infrastructure.
--
-- Source: vendor/archon/migration/complete_setup.sql
-- Version: 0.1.0 (complete setup - includes all migrations 001-009)
...
For full prompt details, see vendor/archon/migration/complete_setup.sql lines 1088-1192', 'System prompt for DocumentAgent to create structured documentation following the Document-Builder pattern'),
...
For full prompt details, see vendor/archon/migration/complete_setup.sql lines 1194-1246', 'System prompt for creating feature plans in the features array'),
...
For full prompt details, see vendor/archon/migration/complete_setup.sql lines 1248-1292', 'System prompt for creating data models in the data array')
ON CONFLICT (prompt_name) DO NOTHING;

-- =====================================================
-- ARCHON INTEGRATION COMPLETE
-- =====================================================
-- Archon schema successfully integrated into Supabase project!
--
-- What was added:
-- - 10 archon_* tables (settings, sources, crawled_pages, code_examples,
--   projects, tasks, project_sources, document_versions, migrations, prompts)
-- - 8 vector search functions (multi-dimensional + hybrid search)
-- - 2 helper functions for embedding dimension detection
-- - 1 soft delete function for tasks
-- - 2 custom enums (task_status, task_priority)
-- - 3 PostgreSQL extensions (vector, pgcrypto, pg_trgm)
-- - 15 RLS policies for secure access control
-- - Comprehensive indexes for performance
--
-- Next steps:
-- 1. Generate TypeScript types: npm run types:generate
-- 2. Test RLS policies: npm run test:rls
-- 3. Verify schema in Supabase Studio: http://localhost:8000
-- 4. Review issue #64 for integration best practices
-- =====================================================
