...
function rehypeComponent() {
  return async (tree) => {
    visit(tree, (node) => {
      const { value: srcPath, name: componentName } = getNodeAttributeByName(node, "src") || {};
...
var NestedProperties = defineNestedType(() => ({
      ...
      [
        ...
        // rehypePrettyCodeOptions,
        {
          getHighlighter: async () => {
            const theme = await loadTheme(path2.join(process.cwd(), "/lib/themes/supabase-2.json"));
...
//# sourceMappingURL=compiled-contentlayer-config-VGJ72TEC.mjs.map
